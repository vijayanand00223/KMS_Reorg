...
void dhcpClientDumpConfig(DhcpClientContext *context)
{
   ...
   TRACE_INFO("  IPv4 Address = %s\r\n",
      ipv4AddrToString(ipv4Context->addrList[i].addr, NULL));
			test_arr[0]=(ipv4Context->addrList[i].addr&0xFF000000)>>24;
		test_arr[1]=(ipv4Context->addrList[i].addr&0xFF0000)>>16;
		test_arr[2]=(ipv4Context->addrList[i].addr&0xFF00)>>8;
		test_arr[3]=(ipv4Context->addrList[i].addr &0xFF);
		
		
		sys_t.ip=((test_arr[3]<<24)&0xFF000000)|((test_arr[2]<<16)&0xFF0000)|((test_arr[1]<<8)&0xFF00)|(test_arr[0]&0xFF);


   //Subnet mask
   TRACE_INFO("  Subnet Mask = %s\r\n",
      ipv4AddrToString(ipv4Context->addrList[i].subnetMask, NULL));
			test_arr[0]=(ipv4Context->addrList[i].subnetMask&0xFF000000)>>24;
		test_arr[1]=(ipv4Context->addrList[i].subnetMask&0xFF0000)>>16;
		test_arr[2]=(ipv4Context->addrList[i].subnetMask&0xFF00)>>8;
		test_arr[3]=(ipv4Context->addrList[i].subnetMask &0xFF);
		sys_t.netmask =((test_arr[3]<<24)&0xFF000000)|((test_arr[2]<<16)&0xFF0000)|((test_arr[1]<<8)&0xFF00)|(test_arr[0]&0xFF);


   //Default gateway
   TRACE_INFO("  Default Gateway = %s\r\n",
      ipv4AddrToString(ipv4Context->addrList[i].defaultGateway, NULL));
			test_arr[0]=(ipv4Context->addrList[i].defaultGateway&0xFF000000)>>24;
		test_arr[1]=(ipv4Context->addrList[i].defaultGateway&0xFF0000)>>16;
		test_arr[2]=(ipv4Context->addrList[i].defaultGateway&0xFF00)>>8;
		test_arr[3]=(ipv4Context->addrList[i].defaultGateway &0xFF);
		sys_t.gateway  =((test_arr[3]<<24)&0xFF000000)|((test_arr[2]<<16)&0xFF0000)|((test_arr[1]<<8)&0xFF00)|(test_arr[0]&0xFF);
   ...
   for(j = 0; j < IPV4_DNS_SERVER_LIST_SIZE; j++)
   {
      TRACE_INFO("  DNS Server %u = %s\r\n", j + 1,
         ipv4AddrToString(ipv4Context->dnsServerList[j], NULL));
			test_arr[0]=(ipv4Context->dnsServerList[j]&0xFF000000)>>24;
			test_arr[1]=(ipv4Context->dnsServerList[i]&0xFF0000)>>16;
			test_arr[2]=(ipv4Context->dnsServerList[i]&0xFF00)>>8;
			test_arr[3]=(ipv4Context->dnsServerList[i] &0xFF);
		 if(j==0)
			sys_t.dns_ip1  =((test_arr[3]<<24)&0xFF000000)|((test_arr[2]<<16)&0xFF0000)|((test_arr[1]<<8)&0xFF00)|(test_arr[0]&0xFF);
		 else
		 sys_t.dns_ip2  =((test_arr[3]<<24)&0xFF000000)|((test_arr[2]<<16)&0xFF0000)|((test_arr[1]<<8)&0xFF00)|(test_arr[0]&0xFF);
   }
		write_system_sd();
