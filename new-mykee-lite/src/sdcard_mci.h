...
extern volatile u32_t MCI_CardType;
extern volatile u32_t MCI_Block_End_Flag;
extern volatile u8_t *writevalue;
...
extern u32_t MCI_Init( void );
extern void  MCI_SendCmd( u32_t CmdIndex, u32_t Argument, u32_t ExpectResp, u32_t AllowTimeout );
extern u32_t MCI_GetCmdResp( u32_t CmdIndex, u32_t NeedRespFlag, u32_t *CmdRespStatus );

extern void  MCI_Set_MCIClock( u32_t clockrate );
extern u32_t SD_Set_BusWidth( u32_t width );

extern u32_t MCI_CardInit( void );
extern u32_t MCI_Go_Idle_State( void );
extern u32_t MCI_Check_CID( void );
extern u32_t MCI_Set_Address( void );
extern u32_t MCI_Send_CSD( void );
extern u32_t MCI_Select_Card( void );
extern u32_t MCI_Send_Status( void );
extern u32_t MCI_Set_BlockLen( u32_t blockLength );
extern u32_t MCI_Send_ACMD_Bus_Width( u32_t buswidth );
extern u32_t MCI_Send_Stop( void );

extern u32_t MCI_Write_Block( u32_t blockNum );
extern u32_t MCI_Read_Block( u32_t blockNum );
#endif /* end __MCI_H */
